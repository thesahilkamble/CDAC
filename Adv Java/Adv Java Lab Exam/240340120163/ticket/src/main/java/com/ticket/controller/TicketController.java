...
public class TicketController {
	...
	// Controller Method to Get resolved Tickets with response status 200
	@GetMapping
	public ResponseEntity<?> getOpenTickets(@PathParam(value = "status") Status status ) {
		return ResponseEntity.status(HttpStatus.OK).body(ticketService.getResolvedTickets(status));
	}
